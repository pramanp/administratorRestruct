(function () {
    ...
    angular.
            module('vitricon').
            directive('vcVirtualGrid', vcVirtualGridDirective);

    function vcVirtualGridDirective(Calculation, Session, $q, $timeout) {
        var ddo = {
            restrict: 'E',
            scope: false,
        ...
        function _prelinkfn(scope, el) {
            ...
            scope.options = {
                ...
                selectable: false,
                deleteable: false,
				noDataInGridMsg:'sysadmin.calculation.nocontentmsg'
            };
            scope.events  = {
            ...
            scope.init = function (api) {
                ...
                table.find('tbody').on('click', 'input.select-right', function (event) {
                    ...
                    if(angular.isUndefined(rowsChanged[rowIndex]))
                    ...
                    clrTimout = $timeout(function (){
                        ...
                        angular.forEach(rowsChanged, function(val, key){
                            if(val){
            ...
            scope.read = function () {
                var selectedRealtyId =  Session.getSelectedRealtyId('selectedRealtyId');
                var deferment = $q.defer();
                    Calculation.one(selectedRealtyId).customGET().then(function(resp){
