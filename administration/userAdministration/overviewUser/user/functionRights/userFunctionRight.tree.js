...
(function () {
    angular.module('vitricon').directive('vcSysuserUserFunctionRightsTree', ddoDirective);
    ...
    function ddoDirective($q, $stateParams, User, SystemUser, Mandator, $state) {
        ...
        function _prelinkfn(scope, el, attr) {
            ...
            scope.ancestors = function () {
                ...
                SystemUser.getFunctionRightChildren($stateParams.userdata, 0, {'is-for-group': false}).then(function (response) {
            ...
            scope.load = function (event, data) {
                ...
                SystemUser.getFunctionRightChildren($stateParams.userdata, id, {'is-for-group': false}).then(function (response) {
                    deferment.resolve(scope.covertToTreeData(response.data, {title: 'name', key: 'id', lazy: '.hasChild', selected : '.status'}));
                });
                return deferment.promise;
            };
            ...
            scope.$on('user-to-group-save', function () {
                var dataObj = [];
                ...
                scope.$emit('transfer-nodes-to-group', nodeColletion);
            });
            ...
            scope.$on('node-to-user', function (event, nodes) {
                _.each(nodes, function(node){
                    __mergeNode(node);
                });
            });
